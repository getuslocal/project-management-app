...
const generateCreatedIssuesData = (tickets) => {
  ...
  for (let i = 0; i < 12; i++) {
    const matchedTickets = tickets.filter(ticket => {
      return moment(ticket.createdAt).isBetween(moment(startDate).subtract(14, 'days'), startDate, 'days', '[]');
    });
    startDate.subtract(14, 'days');
...
const generateCompletedIssuesData = (tickets) => {
  ...
  for (let i = 0; i < 12; i++) {
    const matchedTickets = tickets.filter(ticket => {
      ...
      return moment(ticket.completedAt).isBetween(moment(startDate).subtract(14, 'days'), startDate, 'days', '[]');
    });
    startDate.subtract(14, 'days');
    issues.unshift(matchedTickets.length)
  }
