...
import { Top, DayName, Bottom, Week } from './CalendarContent.style';
...
const getCalendarContent = () => {
  ...
  while (
    dateEnd.diff(dateStart, 'weeks') >= 0 &&
...
const DayCellMemo = React.memo((props) => {
  ...
  return <DayCell momentDate={moment(momentDate)} />;
});

const CalendarContent = ({ setLoading, setCurrentMonth, weekCellRef }) => {
  ...
  const onChange = (isVisible, firstDayOfWeek) => {
    ...
    const isFirstWeekOfNextMonth = firstDayOfWeek.isSame(
      moment(firstDayOfWeek).add(1, 'months').startOf('month'),
      'week'
    );
  ...
  const getWeekCellRef = (momentDate, el) => {
    const isFirstDayOfMonth = momentDate.isSame(
      moment(momentDate).startOf('month'),
      'day'
    );
    const isFirstWeekOfNextMonth = momentDate.isSame(
      moment(momentDate).add(1, 'months').startOf('month'),
      'week'
    );
    if (isFirstDayOfMonth) {
      return (weekCellRef.current[momentDate.format('YYYY-MM')] = el);
    } else if (isFirstWeekOfNextMonth) {
      return (weekCellRef.current[
        moment(momentDate).add(1, 'months').format('YYYY-MM')
      ] = el);
    } else {
      return null;
    }
        ...
        {calendar.map((firstDayOfWeek) => (
          ...
          >
            <Week ref={(el) => getWeekCellRef(firstDayOfWeek, el)}>
              ...
              <DayCellMemo
                momentDate={moment(firstDayOfWeek).add(1, 'days').format('LL')}
              />
              <DayCellMemo
                momentDate={moment(firstDayOfWeek).add(2, 'days').format('LL')}
              />
              <DayCellMemo
                momentDate={moment(firstDayOfWeek).add(3, 'days').format('LL')}
              />
              <DayCellMemo
                momentDate={moment(firstDayOfWeek).add(4, 'days').format('LL')}
              />
              <DayCellMemo
                momentDate={moment(firstDayOfWeek).add(5, 'days').format('LL')}
              />
              <DayCellMemo
                momentDate={moment(firstDayOfWeek).add(6, 'days').format('LL')}
              />
            </Week>
          </VisibilitySensor>
        ))}
