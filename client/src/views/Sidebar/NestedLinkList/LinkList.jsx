...
const LinkList = ({ id, linkUrl, icon, title, hasSubMenu, match }) => {
  ...
  const isActive =
    !board && !linkUrl ? 'active' : board === linkUrl.substr(1) ? 'active' : '';
    ...
    <>
      <SidebarList
        ...
        className={`${isActive ? 'active' : ''} ${
          isVisible ? 'open-sub-list' : ''
        }`}
      >
        {hasSubMenu ? (
          <span className={`${icon} ${hasSubMenu ? 'icon-sort-down' : ''}`}>
            {title}
          </span>
        ) : (
          <Link
            to={`/app/${linkUrl}`}
            className={`${icon} ${hasSubMenu ? 'icon-sort-down' : ''}`}
          >
            {title}
          </Link>
        )}
        {hasSubMenu ? (
          <SidebarSubList className={isVisible ? 'visible' : ''}>
            {hasSubMenu.map((menu, index) => (
              <li key={index}>
                <Link to={`/app/${linkUrl}/${menu.linkUrl}`}>
                  {' '}
                  {menu.label}
                </Link>
              </li>
